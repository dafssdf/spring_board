...
public class FileSchedule {
    ...
    * cron 표현식
    * 6~7개의 정보를 표현한다.
    * 초 분 시 일 월 요일 년(년도는 생략가능)
    * *: 와일드 카드(매번)
    * ? : 설정값이 없음(일, 요일 에서만 사용 가능)
...
//        이전 파일들의 전체 경로를 List<Path>타입으로 지정한다.
...
//        파일 객체는 촐더안에 있는 모든 파일 목록을 불러오는 기능이 있다. listFiles()
...
//    1일전의 파일 경로를 만들어주는 메소드
    private String getUploadPathOldFile(){
        ...
        yesterday.add(Calendar.DATE,-1); // Calendar.DATE 에서 -1을 해준다.
