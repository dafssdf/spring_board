...
@Transactional //내부 파일에 있는 메소드는 적용이 안되고, 외부 클래스에 있는 메소드는 트랜잭션 적용이 가능하다.
...
//    삭제
    public void remove(Long boardNumber){
        if(boardNumber == null){
            throw new IllegalArgumentException("존재 하지 않는 게시물");
        }
...
//    조회
    ...
    public BoardVo findBoard(Long boardNumber){
        ...
        return Optional.ofNullable(boardMapper.select(boardNumber)).orElseThrow(() -> {throw new IllegalArgumentException("존재하지 않는 게시물 번호");});
