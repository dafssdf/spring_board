...
class UserControllerTest {
    ...
    * WebApplicationContext는 ApplicationContext는를 상속 받은 타입으로, 웹 어플리케이션과 관련된
    * 정보를 가진다.
    * 서버를 실행하지 않고 컨트롤러를 확인하려면 가짜 요청을 보내야 한다.
    * 가짜 요청을 보내도 스프링 컨테이너가 방응하여 빈 객체를 생성해야 컨트롤러 테스트가 가능하다.
    * WebApplicationContext는 스프링 웹 어플리케이션 관련 정보를 가진 객체이며
    * 요청과 응답 처리, 서블릿 컨데이너 설정, 빈 객체 관리 등을 할 수 있다.
    ...
    * MockMvc객체를 만들기 위해 필요한 것이 WebApplicationContext객체 이다.
    * */
